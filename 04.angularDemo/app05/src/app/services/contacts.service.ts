...
const apiUrl:string = "http://localhost:9090/contacts";
...
export class ContactsService {
  ...
  loadData(){
    ...
    this.httpClient.get<Contact[]>(apiUrl).subscribe({
      next: data => {
  ...
  addContact(c:Contact) {
    ...
    this.httpClient.post<Contact>(apiUrl,c).subscribe({
      next: data => {
  ...
  replaceContact(c:Contact) {    
    this.msgService.setMsg("Please wait while saving...!");
    this.httpClient.put<Contact>(apiUrl +"/"+ c.id,c).subscribe({
      next: data => {
  ...
  removeContactById(id:number) : void {
    ...
    this.httpClient.delete<void>(apiUrl +"/"+ id).subscribe({
      next: () => {
